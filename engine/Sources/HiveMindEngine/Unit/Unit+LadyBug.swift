		...
		while toVisit.isNotEmpty {
			...
			currentPosition.adjacent()
				.filter {
					...
					return visited.contains($0) == false && // Unit cannot backtrack
						currentPosition.freedomOfMovement(to: $0, startingHeight: currentStack.endIndex, endingHeight: targetStack.endIndex, in: state) // Unit can freely move to the target position
					...
					if distanceToRoot == distanceOnHive {
						...
						hivePosition.adjacent().filter { downPosition in
							guard let hiveStack = state.stacks[hivePosition], let downStack = state.stacks[downPosition] else { return false }
							return state.playableSpaces.contains(downPosition) &&
								hivePosition.freedomOfMovement(to: downPosition, startingHeight: hiveStack.endIndex, endingHeight: downStack.endIndex, in: state)
